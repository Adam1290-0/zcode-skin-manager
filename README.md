...
|---|---|---|---|
| **v2.0.12（最新）** | **3.7.6 ~ 3.12.3** | ✅ 当前维护版本 | 🐛 根因修复：失败红点光晕选择器错位 |
| v2.0.11 | 3.7.6 ~ 3.11.2 | ✅ | 🐛 失败红点光晕改柔和 blur 发光 |
| v2.0.10 | 3.7.6 ~ 3.11.2 | ✅ | 🐛 修复字体重叠 + 光晕可视化 |
| v2.0.9 | 3.7.6 ~ 3.11.2 | ✅ | 🎛️ 面板紧凑两列 + 随机配色 + 氛围预设 + 失焦暂停 |
| v2.0.8 | 3.7.6 ~ 3.11.2 | ✅ | 🐛 修复失败红点呼吸光晕不生效 |
| v2.0.7 | 3.7.6 ~ 3.11.2 | ✅ | 🌠 新增彗星模式：单亮头拖尾绕框跑 |
| v2.0.6 | 3.7.6 ~ 3.11.2 | ✅ | 🌈 流动感修复：灯头沿框体绕行 |
| v2.0.5 | 3.7.6 ~ 3.11.2 | ✅ | 🎛️ 扩大氛围灯可调范围（速度/亮度/宽度）；验证适配 ZCode 3.11.2 |
| v2.0.4 | 3.7.6 ~ 3.10.2 | ✅ | 🐛 修复光轨断裂 + 失败呼吸灯无效 |
| v2.0.3 | 3.7.6 ~ 3.10.2 | ✅ | 🌈 氛围灯升级：热点流动 + 亮度呼吸 |
...
- ✅ 验证适配 ZCode 3.12.2 / 3.12.3（核心 DOM 锚点、14 个 CSS 变量、失败红点选择器全部存活，仅重打补丁，无需代码改动）
...
- 🐛 修复失败红点呼吸光晕不生效：去除错误的 opacity 关键帧（它把红点本体一起淡没）、光晕改半透明 rgba 波纹扩散、红点用 scale 脉动

### v2.0.7

- 🌠 新增「彗星模式」开关：单个亮头拖着渐隐长尾绕框跑（经典跑马灯），与三灯头流动模式可切换

### v2.0.6

- 🌈 流动感修复：渐变改为亮暗不均——每段色标中部是混白提亮的「灯头」、段界是降 alpha 的暗谷（同色相不断裂），旋转时灯头沿框体绕行
- 🎛️ 新增「流动强度」滑杆（0–80%，控制灯头与暗谷对比度）

### v2.0.5

- 🎛️ 扩大氛围灯可调范围：速度 1–120s、亮度 0–100%、光轨宽度 0.5–8px、光晕扩散 0–60px、呼吸周期 1–30s
- ✅ 验证适配 ZCode 3.11.2

### v2.0.4

- 🐛 修复光轨断裂：色标间插入中间色 stop，消除深色 UI 上的暗谷
- 🐛 修复失败呼吸灯无效：动画参数直接内联到 keyframes
