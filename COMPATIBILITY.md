...
本补丁**已在 ZCode 3.7.6 / 3.8.1 上开发和验证**。ZCode 3.7.6 对应 Electron 41.0.3；ZCode 3.8.1 的 Electron 版本以你本地安装为准。

- ✅ 已测试版本：**ZCode 3.7.6 / 3.8.1**
- ❌ **其他版本不保证可用**。ZCode 是闭源应用，其内部实现（CSS 变量名、主题 class、`app.asar` 结构、preload API）会随版本变化，任何改动都可能导致本补丁失效甚至让 ZCode 无法启动。

**使用前请先确认你的 ZCode 版本是 3.7.6 或 3.8.1。** 若版本不符，请勿直接打补丁，等待适配或自行评估风险。
...
| **ZCode 版本** | **3.7.6（Electron 41.0.3）/ 3.8.1 — 已验证版本** |
| 安装路径 | `H:\Zcode\`（默认安装位置） |
